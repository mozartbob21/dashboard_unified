...
def test_scanned_pdf_page_looks_the_same_after_extraction(tmp_path):
    ...
    with pymupdf.open() as source:
        ...
        page.insert_image(pymupdf.Rect(20,20,280,380),stream=image.getvalue())
        before = page.get_pixmap().samples
        data = source.tobytes()
    _, data = result(tmp_path,'extract','1',files=[('scan.pdf',data)])
    with pymupdf.open(stream=data) as output:
...
def test_filled_form_values_survive_copying(tmp_path):
    with pymupdf.open() as source:
        ...
        widget.field_value='Filled value'; widget.rect=pymupdf.Rect(30,30,220,70)
...
def test_password_invalid_file_and_page_limits():
    with pymupdf.open(stream=document()) as doc:
        locked = doc.tobytes(encryption=pymupdf.PDF_ENCRYPT_AES_256, owner_pw='test-owner', user_pw='test-reader')
