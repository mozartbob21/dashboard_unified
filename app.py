...
# ===============================
# USERS ADMIN — супер-уровень (только разработчик)
# ===============================
import sqlite3 as _sq3

USERS_DB = BASE_DIR / "data" / "dashboard.db"

def _super_gate(request: Request):
    token = request.cookies.get("access_token")
    user = get_user_from_token(token) or {}
    supers = [x.strip() for x in os.getenv("SUPER_USERS", "boris_st,admin").split(",") if x.strip()]
    if user.get("username") in supers or user.get("role") in ("developer", "super"):
        return user
    return None

def _users_ctx():
    conn = _sq3.connect(str(USERS_DB))
    ts = [r[0] for r in conn.execute("SELECT name FROM sqlite_master WHERE type='table'")]
    t = next((x for x in ts if "user" in x.lower()), None)
    if not t:
        conn.close()
...
def _hash_like(sample, pw):
    import hashlib as _h
    if sample and len(sample) == 64:
...
async def users_page(request: Request):
    if not _super_gate(request):
        return HTMLResponse("Доступ только для разработчика", status_code=403)
    logo = "/static/" + next((p.name for p in (BASE_DIR / "static").glob("*")
             if "logo" in p.name.lower() and p.name.endswith(".svg")), "Logo.svg")
    return templates.TemplateResponse(request, "users_admin.html",
                                      {"request": request, "logo_url": logo})

@app.get("/users/api/list")
async def users_list(request: Request):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    conn, t, cols = _users_ctx()
    if not t:
        return {"error": "таблица пользователей не найдена"}
    cu = _col(cols, "user", "login", "name")
    cr = _col(cols, "role")
    ce = _col(cols, "mail")
    out = []
    for r in conn.execute(f"SELECT * FROM {t}"):
        row = dict(zip(cols, r))
        out.append({"username": row.get(cu, ""), "role": row.get(cr, "user") or "user",
                    "email": row.get(ce, "") or ""})
    conn.close()
    return {"users": out}

@app.post("/users/api/add")
async def users_add(request: Request, payload: dict = None):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    payload = payload or {}
    ...
    if len(uname) < 3 or len(pw) < 6:
        return {"error": "логин ≥ 3, пароль ≥ 6 символов"}
    conn, t, cols = _users_ctx()
    if not t:
        return {"error": "нет таблицы пользователей"}
    cu, cp, cr = _col(cols, "user", "login", "name"), _col(cols, "pass"), _col(cols, "role")
    existing = conn.execute(f"SELECT 1 FROM {t} WHERE {cu}=?", (uname,)).fetchone()
    if existing:
        conn.close()
        return {"error": "такой логин уже есть"}
    sample = (conn.execute(f"SELECT {cp} FROM {t} LIMIT 1").fetchone() or [None])[0]
    vals = {cu: uname, cp: _hash_like(sample, pw)}
    if cr:
        vals[cr] = role
    conn.execute(f"INSERT INTO {t} ({','.join(vals)}) VALUES ({','.join('?' for _ in vals)})",
                 list(vals.values()))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.post("/users/api/{uname}/password")
async def users_pass(request: Request, uname: str, payload: dict = None):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    pw = (payload or {}).get("password") or ""
    if len(pw) < 6:
        return {"error": "пароль ≥ 6 символов"}
    conn, t, cols = _users_ctx()
    cu, cp = _col(cols, "user", "login", "name"), _col(cols, "pass")
    sample = (conn.execute(f"SELECT {cp} FROM {t} LIMIT 1").fetchone() or [None])[0]
    conn.execute(f"UPDATE {t} SET {cp}=? WHERE {cu}=?", (_hash_like(sample, pw), uname))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.post("/users/api/{uname}/role")
async def users_role(request: Request, uname: str, payload: dict = None):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    role = (payload or {}).get("role") or "user"
    conn, t, cols = _users_ctx()
    cu, cr = _col(cols, "user", "login", "name"), _col(cols, "role")
    if not cr:
        conn.close()
        return {"error": "в таблице нет колонки роли"}
    conn.execute(f"UPDATE {t} SET {cr}=? WHERE {cu}=?", (role, uname))
    conn.commit()
    conn.close()
    return {"ok": True}

@app.delete("/users/api/{uname}")
async def users_del(request: Request, uname: str):
    me = _super_gate(request)
    if not me:
        return {"error": "только разработчик"}
    if me.get("username") == uname:
        return {"error": "нельзя удалить самого себя"}
    conn, t, cols = _users_ctx()
    cu = _col(cols, "user", "login", "name")
    conn.execute(f"DELETE FROM {t} WHERE {cu}=?", (uname,))
    conn.commit()
    conn.close()
    return {"ok": True}
...
# ===============================
# MODULE ACCESS v2 — default-deny, список модулей из всех страниц платформы
# ===============================
ACCESS_FILE = BASE_DIR / "data" / "access.json"
...
# Автоматически собираем все страницы платформы как модули
PAGE_MODULES = {"home": "/"}
for _rt in app.routes:
    _p = getattr(_rt, "path", "") or ""
    if (_p and _p != "/" and "{" not in _p
            and not _p.startswith(("/api", "/static", "/docs", "/openapi", "/users"))
            and not any(x in _p for x in ("login", "logout", "auth", "token", "favicon"))):
        PAGE_MODULES[_p.strip("/")] = _p

LABELS = {
    "home": "Главная", "water_dashboard": "Сводный дашборд",
    "watercontrol": "WaterControl", "water-rm": "Вода РМ",
    "aichat": "AI-чат", "prompthub": "ПромтХаб",
    "appeals": "Обращения граждан", "ecur": "ЕЦУР / ДоброДел",
    "cameras": "Камеры", "summarizer": "Суммаризатор", "edo": "ЭДО",
    "water_rm": "Вода РМ",
}
...
async def users_access_get(request: Request, uname: str):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    return {"allowed": _load_access().get(uname, []),
            "modules": {k: LABELS.get(k, "/" + k) for k in PAGE_MODULES}}

@app.post("/users/api/access/{uname}")
async def users_access_set(request: Request, uname: str, payload: dict = None):
    if not _super_gate(request):
        return {"error": "только разработчик"}
    d = _load_access()
    d[uname] = (payload or {}).get("modules") or []
    _save_access(d)
    return {"ok": True}
