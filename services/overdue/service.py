...
def build_public_message(summary, items):
    ...
    lines = [
        "Коллеги, добрый день.",
        "По итогам проверки информационной панели выявлены просроченные задачи.",
...
def build_personal_messages(items):
    ...
    for item in items:
        ...
        message = (
            f"{responsible_name}, добрый день.\n\n"
            f"По данным информационной панели по ОМСУ '{municipality}' "
            f"зафиксировано просроченных задач: {overdue_count}.\n"
            f"Просьба проверить блок '{organization}', отработать просроченные позиции и актуализировать отчёт.\n\n"
            f"Спасибо за оперативную реакцию."
        )
