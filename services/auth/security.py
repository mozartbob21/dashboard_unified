...
from services.auth.keycloak import (  # noqa: E402
    get_user_from_token_keycloak,
    get_user_from_token_keycloak_async,
)
import os  # noqa: E402


def _is_keycloak_token(token):
    """По claim iss определяет, что токен выпущен Keycloak."""
    try:
        import base64
        import json
        ...
        return "/realms/" in str(claims.get("iss", ""))
    except Exception:
        return False

def get_user_from_token(token: str | None) -> dict[str, Any] | None:
    # ── Keycloak path (auto-patched) ──────────────────────
    if _is_keycloak_token(token):
        return get_user_from_token_keycloak(token)
    if not token:
        return None
